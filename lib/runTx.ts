...
async function onBeforeTx(vm, tx) {
  const sioMap = vm.sioMap = {} as any;
 	sioMap.receipts = [] as any[];
	let resolve, reject;
	sioMap.deferred = {
	...
	sioMap.transactions = [];
 	const transactionHash = soliditySha3({
		t: 'bytes',
  	v: bufferToHex(tx.serialize())
 	});
 	const origin = bufferToHex(tx.from);
	sioMap.hashToIndex = {};
	sioMap.hashToIndex[transactionHash] = 0;
  const to = ((v) => v === '0x' ? '0x' + Array(40).fill('0').join('') : v)(bufferToHex(tx.to));
 	sioMap.receipts.push({
 		origin,
 		transactionHash,
		path: [],
		to, 
...
async function _runTx(this: VM, opts: RunTxOpts): Promise<RunTxResult> {
   ...
   */
	await onBeforeTx(this, tx);
  await this._emit('beforeTx', tx)

  // Validate gas limit against base fee
  const basefee = tx.fromMainnet ? new BN(0) : tx.getBaseFee()
  const gasLimit = tx.fromMainnet ? new BN(0) : new BN(tx.gasLimit)
